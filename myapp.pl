...
helper mopidy => sub {
  ...
  $mopidy->on(event => \&process_event);
  $mopidy->on(message => sub { say 'Mopidy message:'; });
...
helper get_tracklist => sub {
  ...
  $c->mopidy->send('tracklist.get_tl_tracks' => sub {
    return unless (my $result = shift->{result});
    p $result;

    $c->populate_tracklist($result);
  });
...
helper send_tracklist => sub {
  ...
  $_->send({ json => {tracklist => $master_tracklist->{array}} })
